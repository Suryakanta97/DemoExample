...
public class ProgrammaticallyManagedLiveTest {
    ...
    private static final String surya_PATH = "/surya";
    ...
    public void givenProgrammaticallyManagedServer_whenUsingSimpleStubbing_thenCorrect() throws IOException {
        ...
        stubFor(get(urlEqualTo(surya_PATH)).willReturn(aResponse().withBody("Welcome to surya!")));

        HttpGet request = new HttpGet("http://localhost:8080/surya");
        HttpResponse httpResponse = httpClient.execute(request);
        String stringResponse = convertResponseToString(httpResponse);

        verify(getRequestedFor(urlEqualTo(surya_PATH)));
        assertEquals("Welcome to surya!", stringResponse);
