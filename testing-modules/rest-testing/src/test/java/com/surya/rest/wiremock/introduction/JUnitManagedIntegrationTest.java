package com.surya.rest.wiremock.introduction;
...
public class JUnitManagedIntegrationTest {
    ...
    private static final String surya_WIREMOCK_PATH = "/surya/wiremock";
    ...
    public void givenJUnitManagedServer_whenMatchingURL_thenCorrect() throws IOException {
        ...
        stubFor(get(urlPathMatching("/surya/.*"))
        ...
        HttpGet request = new HttpGet(String.format("http://localhost:%s/surya/wiremock", port));
        HttpResponse httpResponse = httpClient.execute(request);
        String stringResponse = convertHttpResponseToString(httpResponse);

        verify(getRequestedFor(urlEqualTo(surya_WIREMOCK_PATH)));
    ...
    public void givenJUnitManagedServer_whenMatchingHeaders_thenCorrect() throws IOException {
        stubFor(get(urlPathEqualTo(surya_WIREMOCK_PATH))
    ...
    public void givenJUnitManagedServer_whenMatchingBody_thenCorrect() throws IOException {
        stubFor(post(urlEqualTo(surya_WIREMOCK_PATH))
        ...
        CloseableHttpClient httpClient = HttpClients.createDefault();
        HttpPost request = new HttpPost(String.format("http://localhost:%s/surya/wiremock", port));
        request.addHeader("Content-Type", APPLICATION_JSON);
        request.setEntity(entity);
        HttpResponse response = httpClient.execute(request);

        verify(postRequestedFor(urlEqualTo(surya_WIREMOCK_PATH))
                .withHeader("Content-Type", equalTo(APPLICATION_JSON)));
    ...
    public void givenJUnitManagedServer_whenNotUsingPriority_thenCorrect() throws IOException {
        stubFor(get(urlPathMatching("/surya/.*")).willReturn(aResponse().withStatus(200)));
        stubFor(get(urlPathEqualTo(surya_WIREMOCK_PATH)).withHeader("Accept", matching("text/.*")).willReturn(aResponse().withStatus(503)));
    ...
    public void givenJUnitManagedServer_whenUsingPriority_thenCorrect() throws IOException {
        stubFor(get(urlPathMatching("/surya/.*")).atPriority(1).willReturn(aResponse().withStatus(200)));
        stubFor(get(urlPathEqualTo(surya_WIREMOCK_PATH)).atPriority(2).withHeader("Accept", matching("text/.*")).willReturn(aResponse().withStatus(503)));
