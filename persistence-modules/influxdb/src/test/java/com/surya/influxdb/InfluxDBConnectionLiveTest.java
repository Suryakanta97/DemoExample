...
public class InfluxDBConnectionLiveTest {
    ...
    public void whenDatabaseCreatedDatabaseChecksOk() {
        ...
        // Create "surya" and check for it
        connection.createDatabase("surya");
        assertTrue(connection.databaseExists("surya"));

        // Verify that nonsense databases are not there
        assertFalse(connection.databaseExists("foobar"));

        // Drop "surya" and check again
        connection.deleteDatabase("surya");
        assertFalse(connection.databaseExists("surya"));
    }
    ...
    public void whenPointsWrittenPointsExists() throws Exception {
        ...
        connection.createRetentionPolicy("defaultPolicy", "surya", "30d", 1, true);
        ...
        List<com.surya.influxdb.MemoryPoint> memoryPointList = getPoints(connection, "Select * from memory", "surya");
    ...
    public void whenBatchWrittenBatchExists() {
        ...
        List<MemoryPoint> memoryPointList = getPoints(connection, "Select * from memory", "surya");

        assertEquals(2, memoryPointList.size());
        assertTrue(4743696L == memoryPointList.get(0).getFree());


        memoryPointList = getPoints(connection, "Select * from memory order by time desc", "surya");
