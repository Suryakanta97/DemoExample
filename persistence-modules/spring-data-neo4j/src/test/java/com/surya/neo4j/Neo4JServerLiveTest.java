...
public class Neo4JServerLiveTest {
    ...
    public void standAloneDriver() {
        ...
        session.run("CREATE (surya:Company {name:\"surya\"}) " +
        ...
        Assert.assertEquals(result.next().get("company.name").asString(), "surya");
    ...
    public void standAloneJdbc() throws Exception {
        ...
        try (Statement stmt = con.createStatement()) {
            stmt.execute("CREATE (surya:Company {name:\"surya\"}) " +
                    "-[:owns]-> (tesla:Car {make: 'tesla', model: 'modelX'})" +
                    "RETURN surya, tesla");
            ...
            while (rs.next()) {
                Assert.assertEquals(rs.getString("company.name"), "surya");
            }
        }
        con.close();
    }
