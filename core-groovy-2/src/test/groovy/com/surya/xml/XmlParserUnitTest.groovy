...
class XmlParserUnitTest extends Specification {
    ...
    def "Should add node to existing com.surya.xml using NodeBuilder"() {
        given: "XML object"
        def articles = parser.parse(xmlFile)

        when: "Adding node to com.surya.xml"
        ...
        then: "Node is added to com.surya.xml properly"
        articles.'*'.size() == 5
    ...
    def "Should remove article from com.surya.xml"() {
        given: "XML object"
        def articles = parser.parse(xmlFile)
