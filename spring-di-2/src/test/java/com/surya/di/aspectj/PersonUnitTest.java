...
public class PersonUnitTest {
    ...
    public void givenUnmanagedObjects_whenInjectingIdService_thenIdValueIsCorrectlySet() {
        PersonObject personObject = new PersonObject("surya");
        personObject.generateId();
        assertEquals(1, personObject.getId());
        assertEquals("surya", personObject.getName());

        PersonEntity personEntity = new PersonEntity("surya");
        assertEquals(2, personEntity.getId());
        assertEquals("surya", personEntity.getName());
    }
}