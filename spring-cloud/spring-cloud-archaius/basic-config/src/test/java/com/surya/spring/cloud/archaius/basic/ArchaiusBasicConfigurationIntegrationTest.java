package com.surya.spring.cloud.archaius.basic;
...
public class ArchaiusBasicConfigurationIntegrationTest {
    ...
    private DynamicStringProperty testPropertyWithDynamic = DynamicPropertyFactory.getInstance()
        .getStringProperty("surya.archaius.test.properties.one", "not found!");
    ...
    public void givenIntialPropertyValue_whenPropertyChanges_thenArchaiusRetrievesNewValue() {
        ...
        TestPropertyValues.of("surya.archaius.test.properties.one=new-value")
            .applyTo(context);
        context.publishEvent(new EnvironmentChangeEvent(Collections.singleton("surya.archaius.test.properties.one")));
